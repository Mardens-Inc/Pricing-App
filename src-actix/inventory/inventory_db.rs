...
pub async fn add_record(
    ...
    if let Some(obj) = record.as_object() {
        for (column, value) in obj {
            if column == "id" || column == "date" || column == "last_modified_date" {
                continue; // Skip fields that should be autogenerated
            }
...
pub async fn delete_record(id: u64, record_id: u64, data: &DatabaseConnectionData) -> Result<()> {
    let pool = create_pool(data).await?;
